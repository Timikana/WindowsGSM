...
namespace WindowsGSM.Functions
{
    public class ServerConsole
    {
        ...
        // Bumped on every content change so the UI refresh loop can skip the (expensive) Get() when nothing changed.
        private int _version;
        public int Version => _version;

        private readonly int _serverIdInt;

        public ServerConsole(string serverId)
        {
            _serverId = serverId;
            int.TryParse(serverId, out _serverIdInt);
        }

        public ServerConsole(int serverId)
        {
            _serverId = serverId.ToString();
            _serverIdInt = serverId;
        }

        public void AddOutput(object sender, DataReceivedEventArgs args)
        {
            // BeginInvoke (not Invoke): a chatty server emits hundreds of lines/sec; blocking the stdout
            // reader thread on the UI dispatcher per line stalls it and floods the queue. Ordering on the
            // dispatcher queue is preserved. Server id is parsed once (ctor), not per line.
            System.Windows.Application.Current?.Dispatcher.BeginInvoke(new System.Action(() =>
            {
                MainWindow._serverMetadata[_serverIdInt].ServerConsole.Add(args.Data);
            }));
        }
