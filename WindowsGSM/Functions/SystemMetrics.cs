...
namespace WindowsGSM.Functions
{
    class SystemMetrics
    {
        ...
        // Live usage used to spin up a fresh WMI ManagementObjectSearcher on EVERY call (dashboard polls
        // every 2s, 24/7) — ~10-50ms + undisposed COM objects each time. Replaced by native APIs:
        // a shared PerformanceCounter for CPU, GlobalMemoryStatusEx for RAM, a cached DriveInfo for disk.
        private static PerformanceCounter _cpuCounter;
        private static readonly object _cpuLock = new object();
        private DriveInfo _driveCached;
        ...
        public void GetDiskStaticInfo(string disk = null)
        {
            ...
            _driveCached = DriveInfo.GetDrives().FirstOrDefault(x => (x.Name == disk) && x.IsReady);
            DiskType = _driveCached?.DriveFormat;
            DiskTotalSize = _driveCached?.TotalSize ?? 0;
        }
        ...
        public double GetCPUUsage()
        {
            try
            {
                // Shared static counter read from both the WPF dashboard loop and the Discord loop:
                // PerformanceCounter.NextValue() isn't thread-safe, so serialize the whole read, not just creation.
                lock (_cpuLock)
                {
                    if (_cpuCounter == null)
                    {
        ...
        public double GetRAMUsage()
        {
            try
            {
                ...
                return GlobalMemoryStatusEx(ref m) ? m.dwMemoryLoad : 0; // % of physical memory in use
            }
            catch
            {
                return 0;
        ...
        public double GetDiskUsage(string disk = null)
        {
            try
            {
                ...
                {
                    disk = disk ?? Path.GetPathRoot(Process.GetCurrentProcess().MainModule.FileName);
                    d = DriveInfo.GetDrives().FirstOrDefault(x => (x.Name == disk) && x.IsReady);
                }
                if (d == null || DiskTotalSize == 0) { return 0; }
                return (1 - (double)d.AvailableFreeSpace / DiskTotalSize) * 100;
            }
            catch
            {
                return 0;
