...
namespace WindowsGSM.Functions
{
    public class ServerConfig
    {
            ...
            if (File.Exists(configpath))
            {
              try
              {
                foreach (string line in File.ReadLines(configpath))
                {
                    ...
                    {
                        keyvalue[1] = (keyvalue[1].Length >= 2 ? keyvalue[1].Substring(1, keyvalue[1].Length - 2) : keyvalue[1].Trim('"'));
                        ...
                        switch (keyvalue[0])
                        {
                            ...
                            case SettingName.BackupCrontabFormat: BackupCrontabFormat = keyvalue[1]; break;
                        }
                    }
                }
              }
        ...
        public bool CreateWindowsGSMConfig()
        {
            ...
            if (!File.Exists(configpath))
            {
                ...
                using (TextWriter textwriter = new StreamWriter(configpath))
                {
                    ...
                    textwriter.WriteLine($"{SettingName.MemoryWatchdog}=\"0\"");
                    textwriter.WriteLine($"{SettingName.MemoryLimitMB}=\"8000\"");
                    textwriter.WriteLine($"{SettingName.BackupBeforeUpdate}=\"0\"");
                    textwriter.WriteLine($"{SettingName.BackupCrontab}=\"0\"");
                    textwriter.WriteLine($"{SettingName.BackupCrontabFormat}=\"0 5 * * *\"");
                    textwriter.WriteLine(string.Empty);
        ...
        {
            // RCON = full remote admin of the game server, so use a cryptographic RNG (System.Random is
            // time-seeded and predictable — two servers created close together could even collide).
            ...
            if (File.Exists(configFile))
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (keyvalue.Length == 2)
                    {
                        ...
                        {
                            return (keyvalue[1].Length >= 2 ? keyvalue[1].Substring(1, keyvalue[1].Length - 2) : keyvalue[1].Trim('"'));
                        }
                    }
                }
            }
