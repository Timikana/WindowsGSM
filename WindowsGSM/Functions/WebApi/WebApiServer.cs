...
namespace WindowsGSM.Functions.WebApi
{
    ...
    public class WebApiServer
    {
        ...
        // one build (the underlying data only refreshes every ~5s anyway).
        private readonly object _serversJsonLock = new object();
        private string _serversJsonCache;
        private DateTime _serversJsonAtUtc = DateTime.MinValue;
        private static readonly TimeSpan ServersJsonTtl = TimeSpan.FromSeconds(3);

        private string GetServersJsonCached()
        {
            lock (_serversJsonLock)
            {
                if (_serversJsonCache != null && DateTime.UtcNow - _serversJsonAtUtc < ServersJsonTtl) { return _serversJsonCache; }
            }
            string json = _getServersJson() ?? "[]"; // may Dispatcher.Invoke to the UI thread — done outside the lock
            lock (_serversJsonLock) { _serversJsonCache = json; _serversJsonAtUtc = DateTime.UtcNow; }
            return json;
        }
        ...
        private void Handle(HttpListenerContext ctx, bool isPortal)
        {
            ...
            {
                WriteJson(res, 200, GetServersJsonCached());
                return;
            }
        ...
        private void HandleLogin(HttpListenerRequest req, HttpListenerResponse res, string ip)
        {
            ...
            if (u == null)
            {
                ...
                // No Thread.Sleep here (it blocked an HttpListener pool thread for 400ms per attempt).
                // Brute-force is already covered by the per-IP throttle (10 fails / 5 min) and the constant-
                // time PBKDF2 verify (200k iterations, incl. a dummy hash for unknown users).
                ServePage(res, false, "Invalid credentials.");
                return;
            }
            ResetFails(ip);
            SweepStale(); // opportunistic janitor so _sessions / _failsByIp can't grow unbounded
        ...
        // Opportunistic janitor (called on each login): drop expired sessions and stale throttle entries
        // so neither dictionary grows unbounded over a long-running listener.
        private void SweepStale()
        {
            var now = DateTime.UtcNow;
            foreach (var kv in _sessions) { if (kv.Value.Expiry < now) { _sessions.TryRemove(kv.Key, out _); } }
            foreach (var kv in _failsByIp) { if (now - kv.Value.since > FailWindow) { _failsByIp.TryRemove(kv.Key, out _); } }
        }
