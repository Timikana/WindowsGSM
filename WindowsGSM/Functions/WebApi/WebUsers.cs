...
namespace WindowsGSM.Functions.WebApi
{
    ...
    public class WebUsers
    {
        ...
        // The web dashboard polls the API every 5s; Load() used to re-read the JSON from disk every time.
        // Cache the file TEXT keyed by last-write time (the disk read is the expensive part), but ALWAYS
        // deserialize a FRESH instance per Load — callers (esp. the users dialog) mutate their copy, so
        // handing out a shared instance would race the API's concurrent auth enumeration.
        private static readonly object _cacheLock = new object();
        private static string _cacheText;
        private static DateTime _cacheMtimeUtc;
        ...
        public static WebUsers Load()
        {
            try
            {
                if (File.Exists(Path))
                {
                    ...
                    lock (_cacheLock)
                    {
                        if (_cacheText != null && mtime == _cacheMtimeUtc) { text = _cacheText; }
                        else { text = File.ReadAllText(Path); _cacheText = text; _cacheMtimeUtc = mtime; }
                    }
                    return JsonConvert.DeserializeObject<WebUsers>(text) ?? new WebUsers();
                }
            }
        ...
        public void Save()
        {
            try
            {
                ...
                lock (_cacheLock) { _cacheText = null; } // force a fresh read on the next Load
            }
